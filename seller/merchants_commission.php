	...
	for ($i = 0; $i < $count; $i++) {
		...
		if (file_exists(MOBILE_DRP)) {
			...
			if ($commission_info['commission_model']) {
				...
				$effective_amount_price = $order_commission + $row[$i]['shipping_fee'];
				...
				$effective_amount_price = ($total_return_amount * $percent_value) + $brokerage_amount['should_amount'] + $row[$i]['shipping_fee'];
				$row[$i]['effective_amount_price'] = number_format($effective_amount_price, 2, '.', '');
			}
			...
			if ($commission_info['commission_model']) {
				...
				$effective_amount_price = ($total_return_amount * $percent_value) + $goods_rate['should_amount'] + $row[$i]['shipping_fee'];
				$row[$i]['effective_amount_price'] = number_format($effective_amount_price, 2, '.', '');
			}
