...
function operable_list($order)
{
	...
	if (((OS_CONFIRMED == $os) || (OS_SPLITED == $os)) && (PS_PAYED == $ps) && (SS_UNSHIPPED == $ss)) {
		if ($priv_list['os'] && $priv_list['ss'] && $priv_list['ps']) {
			$list['return'] = true;
		}
	}
...
function order_list()
{
	...
	if ($result === false) {
		...
		$sql = 'SELECT ifnull(bai.is_stages,0) is_stages, o.order_id, o.main_order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid, o.is_delete,' . 'o.shipping_time, o.auto_delivery_time, o.pay_status, o.consignee, o.address, o.email, o.tel, o.mobile, o.extension_code, o.extension_id, o.user_id, o.referer, o.froms, o.chargeoff_status, ' . '(' . order_amount_field('o.') . ') AS total_fee, (o.goods_amount - o.discount + o.tax + o.shipping_fee + o.insure_fee + o.pay_fee + o.pack_fee + o.card_fee) AS total_fee_order, o.pay_id ' . ' FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('baitiao_log') . ' AS bai ON o.order_id=bai.order_id ' . $leftJoin . $where . $where_store . $no_main_order . $groupBy . ' ORDER BY ' . $filter['sort_by'] . ' ' . $filter['sort_order'] . ' ' . ' LIMIT ' . (($filter['page'] - 1) * $filter['page_size']) . ',' . $filter['page_size'];
...
if ($_REQUEST['act'] == 'order_query') {
	...
	array('href' => 'order.php?act=list&' . list_link_postfix(), 'text' => $_LANG['return_list'])
		...
		if ('confirm' == $operation) {
			...
			$vc_id = (isset($_REQUEST['vc_id']) && !empty($_REQUEST['vc_id']) ? intval($_REQUEST['vc_id']) : 0);
			$order_amount = $order['money_paid'] + $order['surplus'];
			if (($order_amount < $return_amount) && (0 < $shipping_fee) && ($shipping_fee <= $order_amount)) {
				$return_amount = $order_amount - $shipping_fee;
			}

			$order_amount = $order_amount - $return_amount;
...
if (!function_exists('update_pay_log')) {
