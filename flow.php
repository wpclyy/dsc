...
if ($_REQUEST['step'] == 'add_to_cart') {
    ...
    exit($json->encode($result));
} else if ($_REQUEST['step'] == 'add_to_cart_showDiv') {//加入购物车，并弹出提示框
    include_once 'includes/cls_json.php';
    $_POST['goods'] = strip_tags(urldecode($_POST['goods']));
    $_POST['goods'] = json_str_iconv($_POST['goods']);
    if (!empty($_REQUEST['goods_id']) && empty($_POST['goods'])) {
...
if ($_REQUEST['step'] == 'add_cart_combo_goodsAttr') {
    ...
    $_SESSION['flow_order'] = $order;
} else if ($_REQUEST['step'] == 'check_surplus') {//生成订单时要检测此段
    $surplus = floatval($_GET['surplus']);
    $user_info = user_info($_SESSION['user_id']);
    ...
    exit();
} else if ($_REQUEST['step'] == 'check_integral') {//生成订单时要检测此段
...
//接收用户提交的订单信息，（抵扣积分数额也在内）
        ...
        if ($pay_online['pay_online']) {
            ...
            $pay_integral = (isset($_REQUEST['integral']) ? addslashes(trim($_REQUEST['integral'])) : '');//接收抵扣积分数额
        ...
        if (0 < $user_id) {
            ...
            $user_info = user_info($user_id);//获取用户详细信息，做会员等地判断所用到的信息都在这里
        ...
        if (0 < $order['order_amount']) {
            ...
            if ($order['pay_id'] == $onlinepay_pay_id) {
                ...
                foreach ($payment_list as $K => $v) {
                    if ($v['is_online'] == 1) {
                        ...
                        if (file_exists($payment_file)) {
                            ...
                            $pay_obj = new $v['pay_code'](); //生成支付请求链接！！！！
                ...
                $smarty->assign('pay_online_button', $pay_online_button);//支付链接生成
        ...
        $is_update_sale = is_update_sale($new_order_id);
        if (($_CFG['sales_volume_time'] == SALES_PAY) || (($_CFG['sales_volume_time'] == SALES_SHIP) && ($is_update_sale == 0))) {
