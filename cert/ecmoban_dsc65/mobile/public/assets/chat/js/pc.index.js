...
var workmanEvent = {
    ...
    init: function () {
        ...
        // 声音
        workmanEvent.audio = new Audio(audio_path);
    ...
    init_service_data : function (data) {
        ...
        for ( i in this.vueobj.service_list ) {
            ...
            if ( this.vueobj.service_list[i].ru_id == workmanKefu.user.store_id || workmanKefu.user.store_id === 0 ) {
                //  列表存在该商家
                is_exist_list = true;
        ...
        if ( is_exist_list == false ) {
            let d = {
                add_time : workmanKefu.SystemDate(),
                count : 0,
                message : "您好",
                ru_id : workmanKefu.user.store_id,
                service_id : 0,
                shop_name : workmanKefu.user.store_name,
                thumb :  workmanKefu.user.store_logo
            };
            this.vueobj.$set(this.vueobj.service_list, 0, d);
            /**
             * 将消息添加到页面
             */
            d.avatar = workmanKefu.user.store_logo;
            d.from_id = d.service_id;
            d.message = d.message;
            d.message_type = '';
            d.name = workmanKefu.user.store_name;
            d.service_id = d.service_id;
            d.store_id = d.ru_id;
            d.time = d.add_time;
            workmanEvent.add_message(d, 2);
        }

        workmanEvent.vueobj.change_service(workmanEvent.vueobj.current_target);
    ...
    service_list_chat_data : function (service_id){
        ...
        $.ajax({
            ...
            data : {
                id : service_id,
                type : "default",
                goods_id : workmanKefu.user.goods_id
            },
            success : function (res) {
                res.chat.reverse();
                if (
    ...
    add_message: function (data, warp_chat) {
        ...
        let d = {
            ...
            user_picture: (warp_chat == 1) ? workmanKefu.user.avatar : data.avatar,
            user_name: data.name,
            add_time: data.time,
    ...
    sendEnterMsg: function () {
        ...
        workmanKefu.message.store_id = workmanEvent.target_service.store_id;
    ...
    vueapp: function () {
        ...
        this.vueobj = new Vue({
            ...
            data: {
                ...
                user_name : workmanKefu.user.user_name,
                user_avatar : workmanKefu.user.avatar,
                service_list : [],
            ...
            methods: {
                change_service : function ( service_id, event ) {
                    ...
                    workmanEvent.service_list_chat_data( service_id );
                    // 未读数量清零
                    ...
                    workmanEvent.target_service = {
                        uid: service_id,
                        store_id : (event == undefined) ? workmanKefu.user.store_id : event.currentTarget.getAttribute("data-ruid")
                    };
                },
                get_more_msg : function () {
                    ...
                    workmanEvent.get_more_msg(page + 1, this.service_list[this.current_target].ru_id);
                }
            },
            computed: {
                evenNumbers: function () {
                    ...
                    return this.service_list_chat_data[this.current_target].chat.filter(function (list) {
                        ...
                        list.user_picture = (list.warp_chat == 2 || list.warp_chat == 'warp-chat-right') ? workmanKefu.user.avatar : ( list.avatar == undefined ) ? list.user_picture : list.avatar
    ...
    get_service: function (data) {
        ...
        if ( this.vueobj.current_target == data.service_id ||  this.vueobj.current_target == 0 ) {
            workmanEvent.target_service.uid = data.service_id;
        ...
        for (let i in this.vueobj.service_list) {
            ...
            if ( this.vueobj.service_list[i].ru_id == data.store_id &&
                ...
                workmanEvent.add_message({
                    ...
                    name : workmanKefu.user.user_name,
                    service_id : data.service_id,
        ...
        data.time = workmanKefu.SystemDate();
        if (data.msg != '') {
            workmanEvent.add_message(data, 2);
        }
    ...
    get_more_msg : function (page, store_id) {
        ...
        $.ajax({
            ...
            success : function (res) {
                let data = workmanKefu.json_encode(res)
