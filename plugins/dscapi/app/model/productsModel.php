...
abstract class productsModel extends \app\func\common {
    ...
    public function get_insert($table, $select, $format) {
        ...
        $common_data = array('result' => empty($id) ? 'failure' : 'success', 'id' => $id, 'msg' => empty($id) ? $productsLang['msg_failure']['failure'] : $productsLang['msg_success']['success'], 'error' => empty($id) ? $productsLang['msg_failure']['error'] : $productsLang['msg_success']['error'], 'format' => $format);
