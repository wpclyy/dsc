...
function order_goods($order_id) {
    $sql = 'SELECT og.rec_id, og.goods_id, og.ru_id, og.goods_name, og.goods_sn, og.market_price, og.goods_number, og.warehouse_id, ' . 'og.goods_price, og.goods_attr, og.is_real, og.parent_id, og.is_gift, ' . 'og.goods_price * og.goods_number AS subtotal,og.dikoucount * og.goods_number AS subdikou,og.dikoubenjin, og.extension_code,g.shop_price, g.is_shipping, g.goods_weight AS goodsWeight, g.goods_img, g.goods_thumb, ' . 'g.goods_cause FROM ' . $GLOBALS['ecs']->table('order_goods') . ' AS og ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON og.goods_id = g.goods_id ' . ' WHERE og.order_id = \'' . $order_id . '\'';
    $res = $GLOBALS['db']->query($sql);
...
function get_order_user_flow_bonus($today, $goods_amount, $user_id, $where, $ru_id = NULL) {
    if (!is_null($ru_id)) {
