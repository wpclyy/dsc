...
function get_order_goods_toInfo($order_id = 0) {
    ...
    foreach ($res as $key => $row) {
        ...
        if ($row['og_extension_code'] == 'presale') {
            ...
            $group_id = $GLOBALS['db']->getOne('SELECT extension_id FROM ' . $GLOBALS['ecs']->table('order_info') . ' WHERE order_id = \'' . $order_id . '\'');
            $arr[$key]['url'] = build_uri('group_buy', array('gbid' => $group_id));
        } else {
