...
function get_wholesale_child_cat($cat_id = 0, $type = 0) {
    ...
    if ($GLOBALS['db']->getOne($sql) || ($parent_id == 0)) {
        $sql = 'SELECT cat_id,cat_name ,parent_id,is_show, style_icon ' . 'FROM ' . $GLOBALS['ecs']->table('wholesale_cat') . 'WHERE parent_id = \'' . $parent_id . '\' AND is_show = 1 ORDER BY sort_order ASC, cat_id ASC';
        $res = $GLOBALS['db']->getAll($sql);
        $cat_arr = array();
